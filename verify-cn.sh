...
if [ "$depth" -eq 0 ]; then
        ...
        clientCN="$(echo "$commonName" |  sed 's/^.*CN=//')"

        # Get the Intune token so that we are allowed to do api calls
        postParams='client_id=client_id&scope=https://graph.microsoft.com/.default&client_secret=client_scret_placeholder&grant_type=client_credentials'
